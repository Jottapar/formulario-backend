...
def create(datos:PersonalCreate, session: Session)-> PersonalRead:
    ...
    if not session.get(Genero,datos.genero_id):
        raise ValueError(f'El genero con el id {datos.genero_id} no existe')
    if not session.get(Alimentacion, datos.alimentacion_id):
        raise ValueError(f'La Alimentacion con el id {datos.alimentacion_id} no existe')
    if not session.get(Ciudad, datos.ciudad_id):
        raise ValueError(f'La ciudad con el id {datos.ciudad_id} no existe')
    if not session.get(Eps, datos.eps_id):
        raise ValueError(f'La eps con el id {datos.eps_id} no existe')
    if not session.get(StatusPersonal, datos.status_personal_id):
        raise ValueError(f'El status personal con el id {datos.status_personal_id} no existe') 
